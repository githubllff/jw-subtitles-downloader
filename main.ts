...
export default class JwSubtitlesPlugin extends Plugin {
  ...
  async sync() {
    ...
    for (const url of urls) {
      ...
      const title = meta(html, 'og:title') || id;
  ...
  async download(id: string): Promise<string | null> {
    ...
    const track = files.find((file: any) => file.subtitles?.url)?.subtitles?.url;
  ...
  async write(item: Item, vtt: string) {
    ...
    if (old instanceof TFile) await this.app.vault.modify(old, text);
  ...
  display() {
    ...
    new Setting(this.containerEl)
      ...
      .addText(text => text.setValue(this.plugin.settings.rootFolder).onChange(async value => {
        this.plugin.settings.rootFolder = value || DEFAULT_SETTINGS.rootFolder;
    ...
    new Setting(this.containerEl)
      ...
      .addText(text => text.setValue(this.plugin.settings.language).onChange(async value => {
        this.plugin.settings.language = value.toUpperCase();
        await this.plugin.saveData(this.plugin.settings);
      }));
  }
}
...
function extractId(value: string): string | null {
  return value.match(/pub-[A-Za-z0-9_-]+/)?.[0] || null;
}

function meta(html: string, property: string): string {
  return html.match(new RegExp(`<meta[^>]+(?:property|name)=["']${property}["'][^>]+content=["']([^"']+)`, 'i'))?.[1] || '';
}
