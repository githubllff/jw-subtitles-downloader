...
interface Settings { rootFolder: string; language: string; requestDelayMs: number; outputMode: OutputMode; }
const DEFAULT_SETTINGS: Settings = { rootFolder: 'JW Subtitles', language: 'E', requestDelayMs: 750, outputMode: 'both' };
...
export default class JwSubtitlesPlugin extends Plugin {
  ...
  async sync() {
    ...
    for (const url of urls) {
      ...
      try {
        ...
        downloaded++; await this.log(source, `OK wrote ${title}`); await sleep(this.settings.requestDelayMs);
      } catch (error) { failed++; await this.log(source, `ERROR ${error instanceof Error ? error.message : String(error)}`); }
    }
  ...
  async download(id: string, source: TFile): Promise<string | null> {
    ...
    const data = (await requestUrl({ url: api })).json; const media = Array.isArray(data.media) ? data.media : []; const files = media[0]?.files || data.files || [];
  ...
  async write(item: Item, vtt: string) {
    const folder = normalizePath(`${this.settings.rootFolder}/${item.year}`); await this.app.vault.createFolder(this.settings.rootFolder).catch(() => undefined); await this.app.vault.createFolder(folder).catch(() => undefined);
    const path = normalizePath(`${folder}/${safe(item.title)}.md`); const plain = vttToPlain(vtt); const sections = this.settings.outputMode === 'vtt' ? `## Subtitles\n\n${vtt.trim()}` : this.settings.outputMode === 'plain' ? `## Transcript\n\n${plain}` : `## Subtitles\n\n${vtt.trim()}\n\n## Transcript\n\n${plain}`;
    const content = ['---', `jwVideoId: ${item.id}`, `title: ${JSON.stringify(item.title)}`, `year: ${item.year}`, `source: ${item.pageUrl}`, `outputMode: ${this.settings.outputMode}`, '---', '', `# ${item.title}`, '', `Source: [JW.ORG](${item.pageUrl})`, '', sections, ''].join('\n');
...
function extractId(value: string): string | null { let decoded = value; try { decoded = decodeURIComponent(value); } catch { /* keep original */ } const direct = decoded.match(/(?:^|[/?#=&])(pub-[A-Za-z0-9_-]+)(?=$|[/?#&])/i)?.[1]; if (direct) return direct; try { const url = new URL(value); for (const part of [url.searchParams.get('lank'), url.searchParams.get('docid'), url.searchParams.get('item')]) { const match = part?.match(/pub-[A-Za-z0-9_-]+/i); if (match) return match[0]; } } catch { /* not a URL */ } return null; }
function vttToPlain(vtt: string): string { return vtt.split(/\r?\n\r?\n/).map(block => { const lines = block.split(/\r?\n/).map(line => line.trim()).filter(Boolean); if (!lines.length || lines[0].toUpperCase() === 'WEBVTT') return ''; const timeIndex = lines.findIndex(line => line.includes('-->')); const textLines = timeIndex >= 0 ? lines.slice(timeIndex + 1) : lines; return textLines.join(' ').replace(/<[^>]+>/g, '').trim(); }).filter(Boolean).join('\n\n'); }
